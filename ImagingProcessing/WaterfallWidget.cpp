...
WaterfallWidget::WaterfallWidget(QWidget *parent, QString className) : QVTKOpenGLWidget(parent),MsigmaBase(className)
{
...
{
    scalarWidget->SetInteractor(this->GetRenderWindow()->GetInteractor());
    scalarWidget->GetScalarBarActor()->SetTitle("RCSF");
    scalarWidget->GetScalarBarActor()->SetLookupTable((vtkScalarsToColors*)lookupTable);
    scalarWidget->On();
}

void WaterfallWidget::setModelBases(std::vector<ModelBases *> modelObjects)
{
    clearModelBase();
...
void WaterfallWidget::addModelBase(ModelBases *modelObject)
{
...
//    }
    ...
    VTKGeometry_1::makeTriangles(lines, rcsfPolyData.getPolyData(), false);
    ...
    rcsfPolyData.getActor()->GetMapper()->SetLookupTable(pColorTable);
    rcsfPolyData.getActor()->GetMapper()->SetScalarRange(min, max);
    rcsfPolyData.getActor()->GetMapper()->ScalarVisibilityOn();
    renderer->AddActor(rcsfPolyData.getActor());
